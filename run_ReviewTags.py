...
for run in range(1+offset, runs + 1):
    ...
    for tag in tags:
        for num_sentences in review_length:
            ...
            prompt += f"""
                ### INPUT ####
                Create three customer reviews for this product: {product}.
                The review should focus on: {tag}.             
                Each review must be {num_sentences} sentences long.
                
                ### OUTPUT FORMAT ####
                Answer in JSON format, where the first key is "product_name", the second key is "review" and the third key is "tags".
                """
            ...
            out_file = f"{tag_path}_s{str(num_sentences)}_run{str(run)}.txt" 
